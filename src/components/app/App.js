...
function App() {
  ...
  return (
    <div className="App">
      <AppHeader/>
      <Switch>
        <Route path = '/' exact render = {() => <CharactersPage nextPageHandler={nextPageHandler} prevPageHandler={prevPageHandler} characters={characters.results}/>}/>
        <Route path = '/newcharacter' exact component = {AddNewCharacterForm}/>
        <Route path = '/:id' render ={({match}) => <CharacterPage characters={characters.results} match={match}/>}/>
      </Switch>
    </div>
  );
}
